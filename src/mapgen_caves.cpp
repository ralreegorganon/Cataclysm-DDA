...
{
    const string_id<overmap_location> potential_cave_network_surface_entrances("potential_cave_network_surface_entrances");
    ...
    for (int i = 0; i < OMAPX; i++) {
        for (int j = 0; j < OMAPY; j++) {
            ...
            if (potential_cave_network_surface_entrances->test(ter(l)) && ter(lsub1) == empty_rock && ter(lsub2) == empty_rock) {
    ...
    const auto cave_route_to = [&](const tripoint & src, const tripoint & dest, const int &width, const int &height, const bool rock_only) {
        const int z = src.z;
        const auto estimate = [&](const pf::node & cur, const pf::node * prev) {
            ...
            if (closest > 0) {
                ...
                for (const auto &node : path.nodes) {
                    ...
                    for (auto &t : targets) {
                        ...
                        if ((cur_t == empty_rock || cur_t == natural_cave_unremarkable) && is_river && t.x == node.x && t.y == node.y) {
                            cur_t = natural_cave_river;
                            continue;
                        }
