                                   ...
                                   int radius )
{
    ...
    sectors.reserve( ( OMAPX / longest_side ) * ( OMAPY / longest_side ) );
    for( int x = 0; x < OMAPX; x += longest_side ) {
        for( int y = 0; y < OMAPY; y += longest_side ) {
            sectors.emplace_back( x, y );
        }
    }
    ...
    for( const auto om : get_overmaps_near( omt_to_sm_copy( center ), omt_to_sm_copy( radius ) ) ) {
        ...
        if( placed ) {
            // We placed it, so no need to continue attempting placement
            // on other overmaps.
            return true;
        }
    }
